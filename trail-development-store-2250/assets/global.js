...
class CartPerformance {
 ...
 document.addEventListener('DOMContentLoaded', function () {
    const scentDataEl = document.getElementById('scent-data');

    if (!scentDataEl) {
      console.warn('scent-data script not found');
      return;
    }

    try {
      const scentData = JSON.parse(scentDataEl.textContent);
      console.log('Scent Data:', scentData);
      console.log('Product Titles:', scentData.products);
    } catch (error) {
      console.error('Failed to parse scent data JSON', error);
    }
  });